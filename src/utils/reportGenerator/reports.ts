...
const getPeriodDates = async (period: ReportPeriod): Promise<{ startDate: Date; endDate: Date }> => {
  const now = new Date();
  let startDate: Date;
...
const addHeader = async (doc: jsPDF, type: ReportType, startDate?: Date, endDate?: Date) => {
   await addLogo(doc);
  doc.setFontSize(20);
...
const generateDependantsReport = async (doc: jsPDF): Promise<void> => {
  ...
  try {
    ...
    const membersWithDependants = members.filter(m => m.dependants && m.dependants.length > 0).length;
    const totalDependants = members.reduce((sum, m) => sum + (m.dependants?.length || 0), 0);
    ...
    members.forEach(member => {
      if (member.dependants && member.dependants.length > 0) {
        member.dependants.forEach((dependant: Dependant) => {
          ...
          const dependantClaims = claims.filter(claim => 
            claim.claimant && 
            claim.claimant.id === dependant.id && 
            claim.claimant.type === 'dependant'
          );

          const claimInfo = dependantClaims.length > 0 
    ...
    const detailsOptions: UserOptions = {
      startY: lastY + 20,
      head: [['Member', 'Dependant', 'Relationship', 'Date of Birth', 'Claims']],
      body: dependantsData,
      theme: 'striped',
      headStyles: { fillColor: [79, 70, 229] },
      didDrawPage: () => {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'payouts': {
        ...
        doc.setFontSize(14);
        doc.text('Detailed Payouts', 20, (doc as any).lastAutoTable.finalY + 20);
      ...
      case 'summary': {
        ...
        autoTable(doc, {
          ...
          body: [
            ...
            ['Pending Contributions', contributions.filter(c => c.status === 'pending').length],
            ['Approved Contributions', contributions.filter(c => c.status === 'approved').length],
            ['Rejected Contributions', contributions.filter(c => c.status === 'rejected').length],
            ['Total Payouts', payouts.length],
            ['Pending Payouts', payouts.filter(p => p.status === 'pending').length],
            ['Approved Payouts', payouts.filter(p => p.status === 'approved').length],
